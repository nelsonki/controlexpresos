...
export class SalidasTableComponent implements OnInit {
  ...
  public exportTo() {
    ...
    if ((this.fechaExcelInicio !== "" && this.fechaExcelFin !== "") || this.cliente || this.sucursal) {
      ...
      window.open(this.api + 'reports/export/' + this.fechaExcelInicio + "_" + this.fechaExcelFin + '?client_id=' + this.cliente + '&branch_id=' + this.sucursal + '&status=en curso');

    } else {
      window.open(this.api + 'reports/export?client_id=' + this.cliente + '&branch_id=' + this.sucursal + '&status=en curso');
