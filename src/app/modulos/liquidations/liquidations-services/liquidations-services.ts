...
@Injectable()
export class LiquidationsServices {
    ...
    getList(fechaInicio?, fechaFin?) {
        let enpoint = '/liquidations';
        const url = this.api + enpoint;
        return this.http2.get(url, { headers: this.autorization });
    }
    getListPorIdLiquidacion(id) {
        let enpoint = '/liquidations/' + id;
        const url = this.api + enpoint;
        return this.http2.get(url, { headers: this.autorization });
    }
    getListTravels(fechaInicio?, fechaFin?) {
        let enpoint = '/travel?date_start=' + fechaInicio + '&date_end=' + fechaFin;
    ...
    updateTravels(id, body) {
        let enpoint = '/travel/' + id;
        return this.http.doPut(this.api, enpoint, body);
    }
    delete(id) {
        let enpoint = '/liquidations/' + id;
    ...
    getReporte(id?) {
        let enpoint = `/report/travels?travel_id=${id}`;
        const url = this.api + enpoint;
        return this.http2.get(url, { headers: this.autorization });
    }
