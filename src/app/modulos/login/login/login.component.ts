...
export class LoginComponent implements OnInit, AfterViewInit {
  ...
  //public apiUsers = environment.users;
  ...
  constructor(public fb: FormBuilder,
   ...
   //public http: HttpService,
    ...
    //public localService: LocalService, public authService: AuthService
    ) { }
  ...
  sendData() {
    ...
    if (this.form.valid) {
      this.http.doPost('login', object, this.apiUsers).subscribe((data: any) => {
        console.log(data);
        ...
        if (error.status === 403) {
          this.toastr.error('Usuario bloqueado', 'Sistema Jakiro');
        }
      }, () => {
